...
open class SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    @IBInspectable
    @objc dynamic open var titleFont: UIFont = .systemFont(ofSize: 12) {
        didSet {
            updateTitleLabel()
        }
    }
    ...
    @IBInspectable dynamic open var selectedTitleColor: UIColor? {
        didSet {
            updateTitleColor()
        }
    }
    ...
    */
	...
	open var textFieldShouldReturn: (() -> Void)?
	open var textFieldDidEndEditing: (() -> Void)?
	open var textFieldDidBeginEditing: (() -> Void)?
	open var textFieldDidChanged: (() -> Void)?
    ...
    */
    ...
    fileprivate func updateTitleColor() {
        ...
        if !isEnabled {
			...
			guard let selectedTitleColor = selectedTitleColor else {
				return
			}
